        ...
        if ($roles) {// если передан параметр роль
            foreach ($roles as $role) {// проходим в цикле массив ролей
                if ($_SESSION['role'] && $_SESSION['role'] == $role) {
                    /*если роль в сессии совпадает с одним из значений массива роль
                    то выходим из функции, редирект не срабатывает и пользователь
                    попадает на страницу. У него есть права на это*/
                    return;
                }
            }
            /*Если роль в сессии не совпадает ни с одним из значений
            в массиве ролей, значит у пользователя нет прав для просмотра текущей страницы
            переводим на страницу 403 Нет доступа*/
            header('Location: ' . url('/web/errors/403.php'));
        }
        /*Если массив ролей не указан, но логин в сессии указан, то пользователь существует
        значит его можно пустить на страницу, поэтому выходим из функции с помощью return,
        чтобы не отработал редирект ниже*/
        return;
    }
    // если сессия пустая и нас нигде не выбросило на предыдущих этапах,
    // то пользователь не авторизован, значит переводим на страницу авторизации
